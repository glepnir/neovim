...
describe('vim.lsp.completion: integration', function()
  ...
  it('publishes again on <BS> only past the leader the list was filtered for', function()
    exec_lua(function()
      vim.o.completeopt = 'menu,menuone,noinsert'
      _G.published = 0
      local complete = vim.api.nvim__complete
      --- @diagnostic disable-next-line:duplicate-set-field
      vim.api.nvim__complete = function(opts)
        if opts.items then
          _G.published = _G.published + 1
        end
    ...
    retry(nil, nil, function()
      eq({ 'hello', 'help', 'hero' }, matches())
    end)
    feed('l')
    retry(nil, nil, function()
      eq({ 'hello', 'help' }, matches())
    end)
    -- Back to "he", which the list was filtered for: the engine has it all.
    feed('<BS>')
    retry(nil, nil, function()
      eq({ 'hello', 'help', 'hero' }, matches())
    end)
    eq(1, exec_lua('return _G.published'))
    -- Past it, "hand" was never handed over.
    feed('<BS>')
    retry(nil, nil, function()
      eq({ 'hand', 'hello', 'help', 'hero' }, matches())
    end)
    eq(2, exec_lua('return _G.published'))
  end)
  ...
  it('reports "discard" for a native completion it takes over', function()
    ...
    retry(nil, nil, function()
      ...
      -- The native candidates stay, ahead of the server's, except one it offered.
      eq(
        { 'help', 'hello' },
        exec_lua(function()
          return vim.tbl_map(function(m)
