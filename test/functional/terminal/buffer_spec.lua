...
describe(':terminal buffer', function()
  ...
  it('handles loss of focus gracefully', function()
    ...
    neq(tbuf, eval('bufnr("%")'))
    feed(':quit!<CR>') -- Should exit the new window, not the terminal.
    eq(tbuf, eval('bufnr("%")'))
  end)
  ...
  it('requires bang (!) to close a running job #15402', function()
    ...
    eq('Vim(wqall):E948: Job still running', pcall_err(command, 'wqall'))
...
describe(':terminal buffer', function()
  ...
  it('no heap-buffer-overflow when sending long line with nowrap #11548', function()
    ...
    fn.feedkeys("4000ai\\<esc>:terminal!\\<cr>")
    assert_alive()
  end)
...
describe('on_lines does not emit out-of-bounds line indexes when', function()
  ...
  it('deleting a terminal buffer #16394', function()
    command('terminal')
    sleep(500)
    command('lua _G.register_callback(0)')
    feed(':bdelete!<CR>')
    eq('', exec_lua([[return _G.cb_error]]))
  end)
end)
