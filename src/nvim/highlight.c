...
static Map(HlAttrKey, int) combine_attr_entries = MAP_INIT;
static Map(HlAttrKey, int) blend_attr_entries = MAP_INIT;
static Map(HlAttrKey, int) blendthrough_attr_entries = MAP_INIT;
...
void clear_hl_tables(bool reinit)
{
  ...
  if (reinit) {
    ...
    highlight_init();
    map_clear(HlAttrKey, &combine_attr_entries);
    map_clear(HlAttrKey, &blend_attr_entries);
    map_clear(HlAttrKey, &blendthrough_attr_entries);
    ...
    map_destroy(HlAttrKey, &combine_attr_entries);
    map_destroy(HlAttrKey, &blend_attr_entries);
    map_destroy(HlAttrKey, &blendthrough_attr_entries);
...
int hl_combine_attr(int char_attr, int prim_attr)
{
  ...
  HlAttrKey combine_tag = HlAttrKey(char_attr, prim_attr);
  int id = map_get(HlAttrKey, int)(&combine_attr_entries, combine_tag);
  if (id > 0) {
    return id;
  }
  ...
  if (id > 0) {
    map_put(HlAttrKey, int)(&combine_attr_entries, combine_tag, id);
  }
...
int hl_blend_attrs(int back_attr, int front_attr, bool *through)
{
  ...
  HlAttrKey combine_tag = HlAttrKey(back_attr, front_attr);
  Map(HlAttrKey, int) *map = (*through
                              ? &blendthrough_attr_entries
                              : &blend_attr_entries);
  int id = map_get(HlAttrKey, int)(map, combine_tag);
  if (id > 0) {
    return id;
  }
  ...
  if (id > 0) {
    map_put(HlAttrKey, int)(map, combine_tag, id);
  }
  return id;
}
