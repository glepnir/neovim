...
///          session is up.  Starting one raises instead, |textlock| included:
///          that is the caller asking for a session rather than a batch that
///          raced the user.
///
/// @param opts  Optional parameters.
...
/// @return The session's id -- the one given, when it was stopped -- or -1.
  ...
  FUNC_API_SINCE(15)
{
  ...
  if (id == 0) {
    ...
    // Raised rather than answered with -1: starting a session is the caller
    // asking for one, where replacing or stopping is a batch that may have
    // raced the user.  A CompleteChanged handler runs under this lock, and the
    // engine is walking the very list it would swap out.
    if (textlock != 0) {
