...
static size_t ins_compl_replaced_len(compl_T *match)
  FUNC_ATTR_PURE FUNC_ATTR_NONNULL_ALL
{
  colnr_T start = ins_compl_match_col(match);
  if (start >= compl_col) {
    return 0;
  }
  // Bounded by the filter text: ins_compl_add_tv() checked the line against it.
  return MIN((size_t)(compl_col - start), ins_compl_filter_str(match).size);
}
  ...
  FUNC_ATTR_PURE FUNC_ATTR_WARN_UNUSED_RESULT FUNC_ATTR_NONNULL_ARG(1)
{
  ...
  if (str == NULL) {
    return true;  // an unfiltered list matches everything
  }
...
/// dest.
/// Puts "prepend_len" bytes of "head" in front of "src".
static void prepend_startcol_text(String *dest, String *src, size_t prepend_len, const char *head)
{
  size_t new_length = prepend_len + src->size;

  dest->size = new_length;
  dest->data = xmalloc(new_length + 1);  // +1 for NUL

  memmove(dest->data, head, prepend_len);
...
static String *get_leader_for_startcol(compl_T *match, bool cached)
{
  ...
  if (base->data != NULL && start != compl_col) {
    // One of the two is 0: the item's column is on one side of compl_col.
    size_t prepend = ins_compl_replaced_len(match);
    size_t skip = start > compl_col ? (size_t)(start - compl_col) : 0;
    if (skip > base->size) {
      return &compl_leader;  // the leader ends before the item's range starts
    }
    // Keyed on the length: two matches at one column cover the same text.
    size_t new_length = prepend + base->size - skip;
    if (cached && new_length == adjusted_leader.size && adjusted_leader.data != NULL) {
      return &adjusted_leader;
    }
    API_CLEAR_STRING(adjusted_leader);
    if (prepend > 0) {
      // From the filter text: the line holds what the match replaces.
      prepend_startcol_text(&adjusted_leader, base, prepend, ins_compl_filter_str(match).data);
    } else {
  ...
  // Below is for a 'cpt' F{func} source, which keeps the column on the source.
  if (match->cp_startcol >= 0 || cpt_sources_array == NULL) {
    goto theend;
  }
  ...
  if (startcol >= 0 && startcol < compl_col) {
    ...
    API_CLEAR_STRING(adjusted_leader);
    prepend_startcol_text(&adjusted_leader, &compl_leader, (size_t)(compl_col - startcol),
                          ml_get(curwin->w_cursor.lnum) + startcol);
    return &adjusted_leader;
  }
theend:
  return &compl_leader;
...
static void set_fuzzy_score(void)
{
  ...
  do {
    ...
    // Against the text each match is filtered by.
...
bool ins_compl_has_preinsert(void)
{
  ...
  // "menuone" and "fuzzy" no longer decide it either: the preview is virtual
  // text, so it needs neither a menu to browse nor a leader to continue.
  if (ins_compl_has_autocomplete() && p_ic && !p_inf) {
    return false;
  }
  return (get_cot_flags() & kOptCotFlagPreinsert) != 0;
}
...
/// What "match" contributes to the leader, from compl_col on.
static String ins_compl_leader_text(compl_T *match)
  FUNC_ATTR_NONNULL_ALL
{
  size_t skip = ins_compl_replaced_len(match);
  if (skip == 0) {
    return match->cp_str;
  }
  String filter = ins_compl_filter_str(match);
  ...
  FUNC_ATTR_NONNULL_ALL
{
  ...
  // Not under 'longest', which writes into the line as matches are added.
  dict_T *const d = tv->v_type == VAR_DICT ? tv->vval.v_dict : NULL;
  bool has_line = compl_lnum > 0 && compl_lnum <= curbuf->b_ml.ml_line_count;
  int startcol = d == NULL || compl_get_longest || !has_line
                       ? -1 : (int)tv_dict_get_number(d, "startcol") - 1;
  const char *filter_text = d == NULL ? NULL : tv_dict_get_string(d, "filter_text", false);
  size_t filter_len = filter_text != NULL ? strlen(filter_text) : 0;

  bool ok = word != NULL && (empty || *word != NUL);
  if (ok && startcol >= 0) {
    const char *line = ml_get(compl_lnum);
    const char *matched_by = filter_len > 0 ? filter_text : word;
    size_t skip = startcol < compl_col ? (size_t)(compl_col - startcol) : 0;
    // Filtering compares the line from "startcol" against "matched_by".
    ok = startcol <= curwin->w_cursor.col && startcol <= ml_get_len(compl_lnum)
         && utf_head_off(line, line + startcol) == 0
         && (skip == 0 || strncmp(matched_by, line + startcol, skip) == 0);
  }
  if (!ok) {
    free_cptext(cptext);
    tv_clear(&user_data);
  ...
  if (filter_len > 0) {
    compl_curr_match->cp_filter_str = cbuf_to_string(filter_text, filter_len);
  }
  return status;
}
...
static void set_compl_globals(int startcol, colnr_T curs_col, bool is_cpt_compl)
{
  if (is_cpt_compl) {
    ...
    if (startcol < compl_col) {
      prepend_startcol_text(&cpt_compl_pattern, &compl_orig_text,
                            (size_t)(compl_col - startcol),
                            ml_get(curwin->w_cursor.lnum) + startcol);
      return;
    } else {
