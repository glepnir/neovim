...
bool ins_compl_has_preinsert(void)
{
  ...
  if (ins_compl_has_autocomplete() && p_ic && !p_inf) {
    return false;
  }
  // "fuzzy" is no longer excluded: the drawer has the match and skips one that
  // does not continue the leader, where the C preview could only insert.
  return (!ins_compl_has_autocomplete()
          ? (cur_cot_flags & (kOptCotFlagPreinsert|kOptCotFlagMenuone))
          == (kOptCotFlagPreinsert|kOptCotFlagMenuone)
          : (cur_cot_flags & kOptCotFlagPreinsert) != 0);
}
...
static bool ins_compl_stop(const int c, const int prev_mode, bool retval)
{
  ...
  if (compl_cont_mode == CTRL_X_WHOLE_LINE) {
    ...
    // Clamped first: taking a match back above shortens the line, and a column
    // left past its end would have dec_cursor() read outside it.
