  ...
  do {
    if (!match_at_original_text(comp)
      ...
      if (!shown_match_ok && !compl_fuzzy_match) {
        ...
        // when no leader and noselect is false auto select the first item
        if(!compl_no_select && (max_fuzzy_score > 0 || lead_len == 0)) {
          shown_match_ok = true;
          cur = 0;
        }
      }
...
static compl_T *find_comp_when_fuzzy(void)
{
  ...
  bool is_backward = compl_shows_dir_backward();
  if (compl_match_array == NULL || (is_forward && pum_selected_item == compl_match_arraysize - 1)
                  || (is_backward && pum_selected_item == 0)){
    return compl_first_match;
  }
  int target_idx = -1;
  if (is_forward) {
    target_idx = pum_selected_item + 1;
  } else if(is_backward) {
    target_idx = pum_selected_item == -1 ? compl_match_arraysize - 1 : pum_selected_item - 1;
  }
  int score = compl_match_array[target_idx].pum_score;
  char *str = compl_match_array[target_idx].pum_text;
                                      ...
                                      int *num_matches)
{
  ...
  while (--todo >= 0) {
    if (compl_shows_dir_forward() && compl_shown_match->cp_next != NULL) {
      compl_shown_match = !compl_fuzzy_match? compl_shown_match->cp_next : find_comp_when_fuzzy();
      ...
      compl_shown_match = !compl_fuzzy_match ? compl_shown_match->cp_prev : find_comp_when_fuzzy();
      found_end |= is_first_match(compl_shown_match);
    } else {
                          ...
                          bool in_compl_func)
{
  ...
  if (compl_leader != NULL && !match_at_original_text(compl_shown_match) && !compl_fuzzy_match) {
