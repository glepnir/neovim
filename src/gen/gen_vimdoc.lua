...
local function render_funs(funs, classes, cfg)
  ...
  for _, f in ipairs(funs) do
    ...
    if f.deprecated and not f.access and not f.nodoc and not f.classvar then
      local fullname = f.name or ''
      if fullname:match('^nvim_') then
        -- C API: prefix with `vim.api.`
        fullname = 'vim.api.' .. fullname
      elseif not fullname:match('^vim%.') and f.module then
        -- lsp / diagnostic / treesitter: fn_xform didn't prefix, do it here
        fullname = f.module .. '.' .. fullname
      end

      if fullname:match('^vim%.') then
        deprecated_fns[fullname] = true
      end
    end
...
local function gen_deprecated()
  local out_path = vim.fs.joinpath(base_dir, 'runtime/lua/vim/_meta/deprecated.gen.lua')

  local fp = assert(io.open(out_path, 'w'))
  fp:write('-- Generated by gen_vimdoc.lua from @deprecated annotations.\n')
  fp:write('-- DO NOT EDIT.\n\n')
  fp:write('return ' .. vim.inspect(deprecated_fns) .. '\n')
  fp:close()

  print(fmt('Wrote %s (%d entries)', out_path, vim.tbl_count(deprecated_fns)))
end
