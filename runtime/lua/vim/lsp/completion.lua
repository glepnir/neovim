...
--- @field id integer? engine session id from nvim__complete(); this and the two
...
local function build_view(session, lnum, line, compl_col, cursor_col)
  ...
  local order = {}
  --- @type table<table, { score: integer?, start: integer, client_id: integer }>
  local by_item = {}
  ...
  for _, client_id in ipairs(ids) do
    for _, item in ipairs(converted[client_id]) do
      ...
      if not (secondary[client_id] and seen[item.word]) then
        ...
        by_item[item] = { score = how.score, start = how.start, client_id = client_id }
      end
    end
  end
  ...
  elseif not has_completeopt('nosort') then
    ...
    stable_sort(matches, function(a, b)
      ...
      return (nonempty(ia.sortText) or ia.label) < (nonempty(ib.sortText) or ib.label)
    end)
  end
