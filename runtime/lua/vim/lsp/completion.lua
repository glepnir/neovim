...
--- table.sort() is not stable, and two items can tie every key the comparator
--- has: the order they were collected in is the last thing separating them.
--- @generic T
--- @param list T[]
...
--- Up to the first line break, which most texts do not have.
--- @param text string
--- @return string
local function first_line(text)
  local eol = text:find('[\r\n]')
  return eol and text:sub(1, eol - 1) or text
end
...
local function get_completion_word(item)
  ...
  elseif item.textEdit then
    local word = first_line(item.textEdit.newText)
    return word, word
  elseif nonempty(item.insertText) then
    ...
    local text = item.insertText --[[@as string]]
...
--- Per response, which is replaced rather than changed: publishing would
--- otherwise copy every item again.
--- @type table<vim.lsp.CompletionResult, lsp.CompletionItem[]>
local resolved_items = setmetatable({}, { __mode = 'k' })
...
---@param fold boolean fold_case(prefix), which the caller asks once per prefix
---@return boolean visible
---@return integer? score `nil` when unranked
local function score_item(value, prefix, fuzzy, scores, fold)
  if fuzzy then
  ...
  if fold then
    return value:sub(1, #prefix):lower() == prefix:lower(), nil
  end
...
local function infer_start_col(text, line, compl_col, cursor_col)
  ...
  -- line has in front of the word: a word made of \k characters rarely does, and
  -- checking that first skips the scan for most of them.  See #30905 -- "-1" or
  -- ".5" can still be read as reaching back.
  --
  -- The answer is monotone in the cursor: a longer line:sub(s + 1, cursor_col)
  -- extends the shorter one, so the workable columns only ever thin out, and the
  -- window below only ever loses its left end.  An item that lands on compl_col
  -- stays there for the rest of the word, whether the scan came up empty or this
  -- turned it away; one that found an "s" needs a single startswith to keep it.
  -- Enough to cache this per item should the scan ever cost too much.
...
--- @param caller_sorts boolean? the caller orders the items itself, so skip it here
--- @return table[] # |complete-items|
--- @return table<table, { score: integer?, start: integer }> # what build_view()
--- orders them by, keyed by the item it made
function M._lsp_to_complete_items(
  ...
  --- Beside the items rather than inside them: build_view() orders across
  --- clients with this, and "user_data" is what v:completed_item and a user
  --- "cmp" see.
  --- @type table<table, { score: integer?, start: integer }>
  local order = {}
  local min_start = nil ---@type integer?
  local fuzzy = has_completeopt('fuzzy')
  local popup = has_completeopt('popup')
  -- Only the selected item's info is ever shown: on_selection_changed() builds
  -- it for that one where it runs, and "preview" reads it straight from here.
  local lazy_info = popup
    and not has_completeopt('preview')
    and buf_handles[bufnr or api.nvim_get_current_buf()] ~= nil
  ...
  local filterable = {}
  --- fold_case() per prefix, for the same reason.
  --- @type table<string, boolean>
  local folds = {}
  ...
  for _, entry in ipairs(prepared) do
    ...
    elseif not filterable[prefix] or (item.textEdit and not item.textEdit.newText) then
      ...
      if folds[prefix] == nil then
        folds[prefix] = fold_case(prefix)
      end
      visible, score = score_item(filter_text, prefix, fuzzy, fuzzy_scores, folds[prefix])
    end
    ...
    if visible then
      ...
      local info = lazy_info and '' or complete_item_info(item, popup)
      local _, info_kind = get_doc(item)
      local info_complete = item.detail ~= nil and item.documentation ~= nil
      ...
      order[completion_item] = { score = score, start = item_start or compl_col }
    end
  end

  if not user_cmp and not caller_sorts then
    -- Once per item rather than per comparison, which a sort does n log n times.
    ...
    -- The order this client's own items go out in, which is the whole of it for
    -- M._convert_results().  build_view() sorts across clients and falls back to
    -- this only where its own keys tie, so "isIncomplete" keeps a partial list
    -- unsorted there rather than here.
    ...
    table.sort(candidates, by_score and function(a, b)
      local sa = order[a].score or 0
      local sb = order[b].score or 0
      if sa ~= sb then
        return sa > sb
      end
...
--- @field id integer? engine session id from nvim__complete(); this and the three
--- below describe one engine session and are set and cleared together
...
--- @field filtered_for string? leader the list the engine holds was filtered for
...
--- @field empty_for? string Leader the cached responses answered nothing for, so
--- that a longer one needs no second look: both the prefix test and
--- matchfuzzypos() only get stricter as it grows.  Holds only while the cache
--- does, and trigger() drops it whenever a round is on its way.
...
function Session:reset(stop)
  ...
  if stop and id then
    -- Last, once nothing is left to clean up: stopping fires CompleteDone.
...
local function build_view(session, lnum, line, compl_col, cursor_col)
  ...
  --- @type table<integer, table<table, { score: integer?, start: integer }>>
  local order = {}
  --- @type table<table, { score: integer?, start: integer, client_id: integer, sort_text: string, pos: integer }>
  local by_item = {}
  local seen = {} --- @type table<string, true>
  for _, client_id in ipairs(ids) do
    local cached = session.responses[client_id]
    converted[client_id], order[client_id] = M._lsp_to_complete_items(
      cached.result,
      compl_col,
      cursor_col,
      client_id,
      ...
      cached.encoding,
      session.bufnr,
      -- Sorted below across clients, which orders each client's items too.
      not has_completeopt('nosort')
    )
  ...
  for _, client_id in ipairs(ids) do
    for _, item in ipairs(converted[client_id]) do
      ...
      if not (secondary[client_id] and seen[item.word]) then
        ...
        -- compl_col, not zero: an item that somehow arrived without one sorts
        -- with the rest rather than ahead of everything.
        local how = (order[client_id] or {})[item] or { start = compl_col }
        local lsp_item = item.user_data.nvim.lsp.completion_item --- @type lsp.CompletionItem
        by_item[item] = {
          score = how.score,
          start = how.start,
          client_id = client_id,
          sort_text = nonempty(lsp_item.sortText) or lsp_item.label,
          pos = #matches,
        }
      end
    end
  ...
  if user_cmp then
    -- Stably, as below: a comparator that ties two items is a comparator the
    -- caller wrote, and table.sort() on one that is not a strict weak order
    -- gives back anything.
    stable_sort(matches, user_cmp)
  elseif not has_completeopt('nosort') then
    ...
    -- scores do not.  Ends on the collected order, so the order is total and one
    -- comparison is one call, where stable_sort() would make two.
    table.sort(matches, function(a, b)
      local x, y = by_item[a], by_item[b]
      ...
      -- Unranked items have no score, and neither does a client that answered
      -- with nothing to filter by; nil sorts with nil.
      if (x.score or 0) ~= (y.score or 0) then
        return (x.score or 0) > (y.score or 0)
      end
      ...
      -- Never "sortText" across clients: it ranks a server's items among its
      -- own.  Whoever started first goes first, as it did before rank existed.
      if x.client_id ~= y.client_id then
        return x.client_id < y.client_id
      end
      if x.sort_text ~= y.sort_text then
        return x.sort_text < y.sort_text
      end
      return x.pos < y.pos
    end)
  end

  -- Builtin candidates first, as they were before the list was replaced.  Only
  -- those the leader still matches, since the engine filters a new session only
  -- once something is typed, and none a server offered too.
  local others = session.other_items or {}
  local prefix = line:sub(compl_col + 1, cursor_col)
  local fuzzy = has_completeopt('fuzzy')
  local scores --- @type table<string, integer>?
  if fuzzy and prefix ~= '' then
    scores = {}
    local words = {} --- @type string[]
    for _, m in ipairs(others) do
      words[#words + 1] = m.word
    end
    local matched = vim.fn.matchfuzzypos(words, prefix)
    for i, word in
      ipairs(matched[1] --[[@as string[] ]])
    do
      scores[prefix .. '\0' .. word] = (matched[3] --[[@as integer[] ]])[i]
    end
  end
  local fold = fold_case(prefix)
  local view = {} --- @type table[]
  for _, m in ipairs(others) do
    if not seen[m.word] and (prefix == '' or score_item(m.word, prefix, fuzzy, scores, fold)) then
      view[#view + 1] = m
    end
  end
...
--- While nvim__complete() starts a session: the CompleteDone it fires is for the
--- completion taken over, not the user leaving ours.
...
local function do_publish(session, ctx)
  -- Nothing matched a shorter leader, so nothing matches this one either: the
  -- alternative is walking every cached item on every keystroke of a word that
  -- has no candidates, which is the most ordinary thing to be typing.
  local leader = ctx.line:sub(ctx.word_col + 1, ctx.cursor_col)
  if
    session.empty_for
    and #leader > #session.empty_for
    and vim.startswith(leader, session.empty_for)
  then
    return
  end
  ...
  if session.id then
    ...
    if #view == 0 then
      -- reset() drops the cache with it, so there is nothing for "empty_for" to
      -- speak about: the next character asks again.
      session:reset(true)
      return
    end
    session.empty_for = nil
    if api.nvim__complete({ id = session.id, items = view }) ~= -1 then
      session.filtered_for = line:sub(col, cursor_col)
      return
    end
    -- It ended while the request was in flight, or the window it belongs to is
    -- not the one entered now; start a new one below.
    session.id, session.col, session.leader, session.filtered_for = nil, nil, nil, nil
  end

  local col = ctx.word_col + 1
  local view = build_view(session, ctx.row - 1, ctx.line, ctx.word_col, ctx.cursor_col)
  if #view == 0 then
    session.empty_for = leader
  ...
  if id > 0 then
    ...
    session.filtered_for = session.leader
  end
end
...
local function request_incomplete(sess)
  ...
  local ident = vim.tbl_get(buf_handles, bufnr, 'identifier') or {}
  local asked = {} --- @type integer[]
  for _, client in pairs(vim.tbl_get(buf_handles, bufnr, 'clients') or {}) do
...
local function trigger(bufnr, clients, ctx, explicit)
  ...
  -- Whether or not the cache is kept: a round is about to change it, so what a
  -- shorter leader once matched says nothing.  The two paths that read this
  -- publish without asking anyone, so they are not touched by clearing it here.
  session.empty_for = nil
...
function Session:on_leader(leader)
  ...
  -- Matching only narrows as a leader grows, so the list the engine holds has
  -- everything a leader extending the one it was filtered for can match.
  if self.filtered_for and vim.startswith(leader, self.filtered_for) then
    return
  end
...
function register_session_autocmds(bufnr)
  ...
  nvim_on('CompleteDone', group, { buf = bufnr }, function(ev)
    ...
    elseif not starting then
      -- Not the completion ours is taking over; the cache holds then.
...
function on_selection_changed(bufnr)
  ...
  if (completed_item.info or '') ~= '' then
    ...
    local info, kind = complete_item_info(user_data.completion_item, true)
    if info ~= '' then
      local selected = vim.fn.complete_info({ 'selected' }).selected --- @type integer
      local windata = api.nvim__complete_set(selected, { info = info })
      update_popup_window(windata.winid, windata.bufnr, kind)
    end
  end
...
local function on_insert_char_pre(handle, bufnr)
  ...
  if not ctx and next(handle.identifier or {}) then
    ...
    if word ~= '' and tonumber(word) == nil and vim.fn.match(line:sub(col + 1), [[^\k]]) ~= 0 then
      ...
      if all_answered and session then
        for id, c in pairs(handle.identifier) do
          ...
          if not session.responses[id] and c:supports_method('textDocument/completion', bufnr) then
            all_answered = false
            break
          end
        end
...
---
--- @note `autotrigger`, `identifier`, `rank` and `secondary` are per client;
--- `convert`, `cmp`, `commit_characters` and `insert_mode` are the buffer's.
--- Either way a field omitted keeps the value a previous call gave it.
---
