...
--- Detect "transformation" edits: the server wants to replace the typed operator
--- with a different one (e.g. `->` to `.` or `.` to `->`).
---
--- @param item_start_byte integer 0-indexed byte position of textEdit start
--- @param server_start_boundary integer 0-indexed byte of the leftmost edit across all items
--- @param new_text string textEdit.newText
--- @param line string full line content
--- @return boolean
local function is_transform_edit(item_start_byte, server_start_boundary, new_text, line)
  if item_start_byte ~= server_start_boundary then
    return false
  end
  local buf_char = vim.fn.strcharpart(line:sub(item_start_byte + 1), 0, 1)
  return buf_char ~= '' and not buf_char:find('%w') and not vim.startswith(new_text, buf_char)
end
...
function M._lsp_to_complete_items(
  ...
  for _, item in ipairs(items) do
    ...
    if server_start_boundary and line and lnum and encoding and item.textEdit then
      ...
      if item_start_byte and item.textEdit.newText then
        is_transform =
          is_transform_edit(item_start_byte, server_start_boundary, item.textEdit.newText, line)
      end
    end
    ...
    if match then
      ...
      if server_start_boundary and line and item_start_byte then
        ...
        elseif is_transform and item.filterText then
          -- Prefix word with the buffer operator text (e.g. "->" or ".") so
          -- that compl_leader (which includes the operator) can prefix-match.
          local buf_prefix = line:sub(server_start_boundary + 1, item_end_byte)
...
local function needs_text_edit(completion_item, completed_word)
  if
    not completion_item.textEdit
    or completion_item.insertTextFormat == protocol.InsertTextFormat.Snippet
    or completed_word == completion_item.textEdit.newText
  then
    return false
  end
  ...
  return range.start.character ~= range['end'].character or range.start.line ~= range['end'].line
end
...
local function on_complete_done()
  ...
  if needs_text_edit(completion_item, completed_item.word) and Context.cursor then
