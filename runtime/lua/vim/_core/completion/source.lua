...
--- @field yield fun() Suspends until the next main-loop tick.  Shorthand for
--- `await(1, vim.schedule)`.
--- @field await fun(argc: integer, fn: function, ...): ... Calls a
--- callback-taking function and suspends until it answers; "argc" is where the
--- callback goes in its arguments.  Use this rather than |vim.async| directly:
--- it is what `drifted()` knows a suspension by.  Check `cancelled()` and
--- `drifted()` after it returns.
...
local function edit_bounds(line, col, prefix, position)
  ...
  local p = position --[[@as lsp.Position]]
  return p.line, start_char, p.character
end
...
local function collect_keyword(a)
  ...
  -- a narrower scan reaches words the budget never got to, and answers with only
  -- those, so the list the client caches for this word shrinks with it.
  local prefix = keyword_prefix(a.line, a.col)
  local plen = #prefix
  local narrow = plen > 0
    and (a.context --[[@as lsp.CompletionContext]]).triggerKind
  ...
  local base_lnum = (a.position --[[@as lsp.Position]]).line + 1
  ...
  for _, buf in ipairs(target_buffers(a.bufnr)) do
    ...
    if api.nvim_buf_is_loaded(buf) then
      ...
      for _, sg in ipairs(segs) do
        local l = sg.from --- @type integer
        while l <= sg.to do
          local last = math.min(l + SCAN_CHUNK - 1, sg.to) --- @type integer
