...
local function still_relevant(a)
  ...
  return pos[1] - 1 == (a.position --[[@as lsp.Position]]).line
...
local function resolve_position(params)
  ...
  -- Read off the inherited class: luals resolves only the first base of the
  -- multiple inheritance lsp.CompletionParams declares.
  local pos = (params --[[@as lsp.TextDocumentPositionParams]]).position
...
local function do_complete(params, sources, cancelled)
  ...
  end, named) --[[@as vim._core.completion.Source[] ]]

  local yielded = false
  local deadline = 0 -- per source, read by args.exhausted()
  local budget = vim.uv.hrtime() + REQUEST_TIMEOUT_MS * 1000000

  -- Typed once the four below are on it, or every field they add reads as
  -- missing from the table constructor.
  local args = {
    bufnr = bufnr,
    line = line,
    col = col,
    position = (params --[[@as lsp.TextDocumentPositionParams]]).position,
    explicit = explicit_run ~= nil,
    context = ctx or { triggerKind = protocol.CompletionTriggerKind.Invoked },
    cancelled = cancelled,
  }
  -- Both set "yielded": that flag is what drifted() answers by, so a handler
  -- reaching past these into vim.async would suspend without it noticing.
  ...
  --- @cast args vim._core.completion.HandlerArgs
  ...
  return { isIncomplete = truncated, items = items } --[[@as lsp.CompletionList]]
end
  ...
  function srv.request(method, params, callback, notify_reply_callback)
    ...
    elseif method == 'textDocument/completion' then
      ...
      local task = async.run(function()
        -- Cast: async.run() infers its return through a generic the annotation
        -- of do_complete() does not reach.
...
function M.enable(enable, bufnr, opts)
  ...
  if enable == false then
    ...
    if client then
      -- Non-nil: "client" was looked up by it.
      local id = assert(client_id)
      vim.lsp.completion.enable(false, id, bufnr)
  ...
  --- @cast opts { sources: vim._core.completion.Source[] }
  ...
  if not client then
    ...
    client_id = vim.lsp.start({
      ...
      cmd = M.cmd,
      -- Asked for here rather than in protocol.lua: this server registers to
      -- scope its trigger characters, and nothing says every other server
      -- should be told the client handles registration.
      capabilities = {
    ...
    -- Non-nil: the branch above either started one or found the existing one.
    local id = assert(client_id)
  ...
  for _, s in ipairs(opts.sources) do
    ...
    local entry = s --- @type vim._core.completion.Source?
    if s.handler then
      ...
      if not vim.lsp.get_client_by_id(assert(s.client_id)) then
  ...
  for _, s in ipairs(sources) do
    -- Every source here has one: enable() filled it in or validated it.
    kept[assert(s.client_id)] = true
  end
  for _, s in ipairs(previous) do
    ...
    then
      ...
      vim.lsp.completion.enable(false, id, bufnr)
      -- Non-nil: the condition above only holds when it has an entry.
      assert(owned[bufnr])[id] = nil
    end
  end
...
function M.get(opts)
  opts = opts or {}
  --- @cast opts vim.lsp.completion.get.Opts
